...
@Component
@AllArgsConstructor
public class SearchTariffByIdHandler {
    private final SearchTariffByIdValidator validator;
    private final TariffRepository repository;
    private final TariffEntityMapper mapper;

    public TariffVo handle(SearchTariffByIdQuery query) {
        validator.validate(query);

        Optional<Tariff> tariff = repository.findById(query.getId());
